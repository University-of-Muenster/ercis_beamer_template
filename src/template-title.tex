...
\setbeamertemplate{title page}{%
  ...
  {%
    ...
    \begin{tikzpicture}%
      ...
      % University logo
      \node[anchor=north west, no sep] at (\leftTextMarginTitle, {\paperheight - 6mm}) {%
        \beamer@cacheUse{title-wwu-logo}{\titleWwuLogo}%
      };

      % ERCIS logo
      \node[anchor=north east, no sep] at ({\paperwidth - \rightTextMargin}, {\paperheight - 6mm}) {%
      ...
      \matrix(m2)[matrix of nodes, no sep,ampersand replacement=\&, nodes={inner sep=0pt, anchor=text, font={\usebeamercolor[fg]{title page title}\usebeamerfont{title page title}}}, text width = {\matrixtextwidth}, anchor=south] at ({.5\matrixtextwidth + \leftTextMarginTitle - .5pt}, {.4\paperheight - 2mm}){
            \inserttitle \\%
      };

      % Subtitle
      \node[anchor=text, no sep, align=flush left, text width={\paperwidth - \leftTextMarginTitle - \rightTextMargin}] (subtitle) at (\leftTextMarginTitle, {.4\paperheight - 6mm}) {%
